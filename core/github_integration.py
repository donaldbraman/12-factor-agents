...
class GitHubIntegrationAgent(BaseAgent):
    ...
    async def _create_issue_hierarchy(self, task_spec: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if parent_issue_num:
                ...
                self._log_github_action("create_issue", f"Created parent issue #{parent_issue_num}")
            ...
            for i, sub_issue_spec in enumerate(sub_issues_spec):
                ...
                if sub_issue_num:
                    ...
                    self._log_github_action("create_sub_issue", f"Created sub-issue #{sub_issue_num}")
    ...
    async def _orchestrate_github_workflow(self, task_spec: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            for i, phase in enumerate(phases):
                ...
                self.set_progress(0.2 + i * 0.6 / len(phases), f"executing_{phase_name}")
    ...
    async def _execute_workflow_phase(self, phase: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_result["success"] = phase_result["actions_completed"] == len(phase_actions)
    ...
    async def _execute_phase_action(self, action: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _create_single_issue(self, title: str, body: str, labels: List[str] = None) -> Optional[int]:
        ...
        try:
            ...
            if "issues/" in result:
                ...
                self._log_github_action("create_issue", f"Created issue #{issue_num}: {title}")
                return issue_num
                
        except Exception as e:
            self._log_github_action("create_issue_failed", f"Failed to create issue: {str(e)}")
            
        return None
    
    async def _create_pull_request(self, title: str, body: str, branch: str = None) -> Optional[int]:
        ...
        try:
            ...
            self._log_github_action("create_pr_failed", f"Failed to create PR: {str(e)}")
    ...
    async def _add_issue_comment(self, issue_number: int, comment: str):
        ...
        try:
            ...
            self._log_github_action("add_comment", f"Added comment to issue #{issue_number}")
        except Exception as e:
            self._log_github_action("add_comment_failed", f"Failed to add comment: {str(e)}")
    
    async def _fetch_repository_issues(self) -> List[Dict[str, Any]]:
        """Fetch all issues for the repository"""
        try:
            result = await self._run_gh_command("issue", "list", "--json", "number,title,state,labels")
            return json.loads(result)
        except Exception as e:
            self._log_github_action("fetch_issues_failed", f"Failed to fetch issues: {str(e)}")
            return []
    ...
    async def _update_issue_status(self, issue: Dict[str, Any], analysis: Dict[str, Any]):
    ...
    def _generate_project_summary(self, result: Dict[str, Any], task_spec: Dict[str, Any]) -> str:
...
class GitHubProjectOrchestrator(ProgressAwareOrchestrator):
    ...
    def __init__(self, repository: str, project_name: str = "github_project", agent_id: str = None):
        ...
        self.register_phase_processor(WorkflowPhase.INITIALIZATION, self._initialize_github_project)
        self.register_phase_processor(WorkflowPhase.ANALYSIS, self._analyze_project_requirements)
        self.register_phase_processor(WorkflowPhase.PROCESSING, self._create_github_structure)
        self.register_phase_processor(WorkflowPhase.APPROVAL, self._request_project_approval)
        self.register_phase_processor(WorkflowPhase.FINALIZATION, self._finalize_github_project)
