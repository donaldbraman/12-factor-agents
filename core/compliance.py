...
class Factor2Validator(FactorValidator):
    ...
    def validate(self, agent: BaseAgent, context: Dict[str, Any] = None) -> Tuple[ComplianceLevel, Dict[str, Any]]:
        ...
        if prompt_dir.exists():
            ...
            details["recommendations"].append("Create external prompt files in prompts/ directory")
...
class Factor10Validator(FactorValidator):
    ...
    def validate(self, agent: BaseAgent, context: Dict[str, Any] = None) -> Tuple[ComplianceLevel, Dict[str, Any]]:
        ...
        agent_methods = [method for method in dir(agent) 
        ...
        if len(agent_methods) <= 10:
            ...
            details["issues"].append(f"Agent has {len(agent_methods)} public methods - consider decomposition")
        ...
        if hasattr(agent, 'pipeline') or hasattr(agent, 'stages'):
            ...
            details["score"] += 0.2
            details["recommendations"].append("Good use of pipeline pattern for decomposition")
        ...
        if agent.tools:
            ...
            if tool_count <= 5:
                ...
                details["issues"].append(f"Agent has {tool_count} tools - consider decomposition")
...
class ComplianceAuditor:
    ...
    def audit_agent(self, agent: BaseAgent) -> Dict[str, Any]:
        ...
        for factor_num, validator in self.validators.items():
            ...
            if compliance in [ComplianceLevel.FULLY_COMPLIANT, ComplianceLevel.MOSTLY_COMPLIANT]:
                compliant_factors += 1
            ...
            audit_report["summary"]["recommendations"].extend(details["recommendations"])
            
            # Collect pin-citer enhancements
            if "pin_citer_enhancements" in details:
                audit_report["pin_citer_enhancements"][factor_num] = details["pin_citer_enhancements"]
        ...
        if avg_score >= 0.9:
            ...
            audit_report["overall_compliance"] = ComplianceLevel.PARTIALLY_COMPLIANT.value
        else:
    ...
    def _audit_pipeline_pattern(self, agent: BaseAgent) -> Dict[str, Any]:
        ...
        if hasattr(agent, 'pipeline'):
            audit["benefits"].append("Pipeline composition supports Factor 10 (Small, Focused Agents)")
            ...
            if hasattr(pipeline, 'stages') and len(pipeline.stages) > 1:
                audit["benefits"].append("Multiple focused stages demonstrate proper decomposition")
    ...
    def _audit_progress_pattern(self, agent: BaseAgent) -> Dict[str, Any]:
        ...
        if hasattr(agent, 'set_progress') and hasattr(agent, 'progress'):
            audit["benefits"].append("Progress tracking enhances Factor 6 (Pause/Resume)")
    ...
    def _audit_error_pattern(self, agent: BaseAgent) -> Dict[str, Any]:
        ...
        if hasattr(agent, 'handle_error') and hasattr(agent, 'error_context'):
            audit["benefits"].append("Enhanced error context supports Factor 9 (Compact Errors)")
        
        return audit
    
    def generate_compliance_report(self, agent: BaseAgent, output_path: Path = None) -> str:
