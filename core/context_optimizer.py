...
class ContextOptimizer:
    ...
    def optimize_context(self, context: Dict[str, Any], task: str) -> ReducedContext:
        ...
        if usage_ratio > self.delegation_threshold:
            ...
            reduced_context = self.remove_delegated_content(
                reduced_context, delegated_tasks
            )
            final_tokens = self.estimate_tokens(reduced_context)

        # Calculate metrics
        reduction_percentage = (original_tokens - final_tokens) / original_tokens * 100
        estimated_cost_savings = self.calculate_cost_savings(
        ...
        self.metrics_history.append(
            {"timestamp": datetime.now().isoformat(), "metrics": metrics.__dict__}
        )
    ...
    def remove_redundant_context(
        self, context: Dict[str, Any], task: str
    ) -> Tuple[Dict, str]:
    ...
    def summarize_verbose_sections(
        self, context: Dict[str, Any], task: str
    ) -> Tuple[Dict, str]:
    ...
    def identify_delegatable_tasks(
        self, context: Dict[str, Any], task: str
    ) -> List[Dict]:
                    ...
                    delegatable.append(
                        {
                            ...
                            "context_subset": self.extract_subtask_context(
                                context, subtask
                            ),
                        }
                    )
    ...
    def remove_delegated_content(
        self, context: Dict[str, Any], delegated: List[Dict]
    ) -> Dict:
    ...
    def is_complex_subtask(self, subtask: Any) -> bool:
        ...
        return (
            len(subtask_str) > 200
            or "step" in subtask_str.lower()
            or any(
                domain in subtask_str.lower()
    ...
    def extract_domain_context(self, context: Dict) -> Dict:
        ...
        return {
            ...
            if any(dk in k.lower() for dk in domain_keys)
        }
    ...
    def get_optimization_report(self) -> Dict:
        ...
        total_original = sum(
            m["metrics"]["original_tokens"] for m in self.metrics_history
        )
        total_reduced = sum(
            m["metrics"]["reduced_tokens"] for m in self.metrics_history
        )
        total_savings = sum(
            m["metrics"]["estimated_cost_savings"] for m in self.metrics_history
        )
        ...
        return {
            ...
            "average_reduction_percentage": sum(
                m["metrics"]["reduction_percentage"] for m in self.metrics_history
            )
            ...
            "delegation_rate": sum(
                ...
                if m["metrics"]["delegation_recommended"]
            )
            / len(self.metrics_history),
        }
...
class ContextOptimizedAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = None, max_context_tokens: int = 8000):
        ...
        self.context_optimizer = ContextOptimizer(
            max_tokens=max_context_tokens, delegation_threshold=0.3
        )
    ...
    async def execute_task(self, task: str) -> ToolResponse:
        ...
        self.log_info(
            f"Context optimized: {optimized.metrics.reduction_percentage:.1f}% reduction"
        )
        self.log_info(
            f"Tokens: {optimized.metrics.original_tokens} → {optimized.metrics.reduced_tokens}"
        )

        # Handle delegated tasks if any
        if optimized.delegated_tasks:
            delegation_results = await self.handle_delegations(
                optimized.delegated_tasks
            )
    ...
    async def spawn_specialized_agent(
        self, agent_type: str, context: Dict
    ) -> BaseAgent:
        """Spawn specialized agent for delegation"""
        # This would create appropriate agent type
        # For now, return a base agent
        agent = BaseAgent(
            agent_id=f"{self.agent_id}_delegate_{len(self.delegated_agents)}"
        )
        agent.set_context(context)
        return agent

    async def process_with_optimized_context(
        self, task: str, optimized: ReducedContext
    ) -> Any:
