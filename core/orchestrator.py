...
from .pipeline import MultiStagePipeline, PipelineDecision
...
class ProgressAwareOrchestrator(BaseAgent):
    ...
    def __init__(self, workflow_name: str = "generic_workflow", agent_id: str = None):
        ...
        self.approval_required_phases = [WorkflowPhase.PROCESSING, WorkflowPhase.FINALIZATION]
    ...
    async def start_workflow_async(self, initial_data: Dict = None) -> ToolResponse:
        ...
        try:
            ...
            for phase in WorkflowPhase:
                ...
                if self.workflow_status in [WorkflowStatus.PAUSED, WorkflowStatus.FAILED]:
                    break
        ...
        try:
            ...
            if phase in self.approval_required_phases:
                ...
                if not approval_result:
                    ...
                    logger.info(f"Workflow paused pending approval for phase: {phase.phase_name}")
                    return
    ...
    async def _default_phase_processor(self, phase: WorkflowPhase):
        ...
        for step in range(phase_steps):
            ...
            step_progress = phase.progress_milestone + (phase_progress * 0.05)  # Small increment
    ...
    def resume_workflow(self) -> ToolResponse:
        ...
        if self.load_checkpoint():
            ...
            return ToolResponse(
                success=False,
                error="Failed to load workflow checkpoint"
            )
    
    async def _continue_from_current_phase(self):
        """Continue workflow execution from current phase"""
        remaining_phases = [p for p in WorkflowPhase 
                           if p not in self.phases_completed]
