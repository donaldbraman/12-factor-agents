...
class AgentRegistry:
    ...
    def __init__(self, registry_path: Union[str, Path] = None):
        ...
        self._version_index: Dict[str, Dict[str, str]] = {}  # name -> version -> agent_id
    ...
    async def register_agent(
        ...
        try:
            ...
            if (metadata.name in self._version_index and 
                metadata.version in self._version_index[metadata.name] and
                not force):
        ...
        try:
            ...
            if keywords:
                ...
                for agent_id in matching_ids:
                    ...
                    if any(kw.lower() in [ak.lower() for ak in agent_keywords] 
                           for kw in keywords):
        ...
        try:
            ...
            if current_checksum != registration.checksum:
                return ToolResponse(
                    success=False,
                    data={},
                    error=f"Agent module has been modified (checksum mismatch)"
                )
            
            # Load module and class
            spec = importlib.util.spec_from_file_location("agent_module", registration.module_path)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
    ...
    async def update_agent_rating(self, agent_id: str, rating: float, review: str = None) -> ToolResponse:
    ...
    async def get_registry_stats(self) -> ToolResponse:
        ...
        try:
            stats = {
                ...
                "average_rating": sum(a.rating for a in self._agents.values()) / len(self._agents) if self._agents else 0,
                "top_capabilities": [],
                "top_agents": []
            }
