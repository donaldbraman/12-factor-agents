...
class SecurityValidator:
            ...
            return SecurityAuditResult(
                passed=len(
                    [
                        ...
                        if v.severity in [SecurityRisk.HIGH, SecurityRisk.CRITICAL]
                    ]
                )
                == 0,
    ...
    def _determine_capabilities(self) -> List[AgentCapability]:
        ...
        critical_violations = [
            v for v in self.violations if v.severity == SecurityRisk.CRITICAL
        ]
        if not critical_violations:
            capabilities.extend(
                [AgentCapability.NATURAL_LANGUAGE, AgentCapability.DATA_ANALYSIS]
            )
...
class SecurityManager:
    ...
    def __init__(
        self, default_policy: SecurityPolicy = None, signature_key: bytes = None
    ):
    ...
    async def audit_agent(
        ...
        try:
            ...
            if metadata:
                validation_result = self._validate_metadata_consistency(
            ...
            return ToolResponse(
                success=False, data={}, error=f"Security audit failed: {str(e)}"
            )

    async def _audit_file(
        self, file_path: Path, policy: SecurityPolicy
    ) -> SecurityAuditResult:
    ...
    async def _audit_directory(
        self, dir_path: Path, policy: SecurityPolicy
    ) -> SecurityAuditResult:
        ...
        for py_file in python_files:
            ...
            if file_result.risk_level.value == "critical":
                ...
                file_result.risk_level.value == "high"
                and max_risk != SecurityRisk.CRITICAL
            ):
                max_risk = SecurityRisk.HIGH
            elif file_result.risk_level.value == "medium" and max_risk not in [
    ...
    def _validate_metadata_consistency(
        self, audit_result: SecurityAuditResult, metadata: AgentMetadata
    ) -> ToolResponse:
        ...
        try:
                ...
                return ToolResponse(
                    ...
                    data={
                        "undeclared_capabilities": [
                            cap.value for cap in undeclared_caps
                        ]
                    },
            ...
            return ToolResponse(
                success=False, data={}, error=f"Metadata validation failed: {str(e)}"
            )
    ...
    def sign_agent(self, agent_path: Union[str, Path]) -> ToolResponse:
        ...
        try:
            ...
            # Create signature
            signature = hmac.new(
                self.signature_key, content, hashlib.sha256
            ).hexdigest()
            ...
            return ToolResponse(
                success=False, data={}, error=f"Agent signing failed: {str(e)}"
            )

    def verify_signature(
        self, agent_path: Union[str, Path], signature: str
    ) -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse(
                success=False, data={}, error=f"Signature verification failed: {str(e)}"
            )
    ...
    async def get_security_report(self, agent_id: str = None) -> ToolResponse:
        ...
        try:
            if agent_id:
                ...
                agent_audits = [
                    a for a in self.audit_history if agent_id in str(a.checksum)
                ]
                if not agent_audits:
                    return ToolResponse(
                        success=False,
            ...
            for risk in SecurityRisk:
                risk_counts[risk.value] = len(
                    [a for a in audits if a.risk_level == risk]
                )
            ...
            report = {
                ...
                "common_violations": dict(
                    sorted(violation_types.items(), key=lambda x: x[1], reverse=True)[
                        :10
                    ]
                ),
            ...
            return ToolResponse(
                success=True, data=report, metadata={"operation": "security_report"}
            )
        ...
        for builtin in dangerous_builtins:
            if hasattr(builtins, builtin):
                ...
                if builtin in self.policy.banned_functions:
                    setattr(
                        builtins, builtin, self._create_restricted_function(builtin)
                    )
