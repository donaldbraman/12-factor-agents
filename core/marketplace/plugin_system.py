...
from .registry import AgentRegistry, AgentMetadata, AgentCapability
...
class PluginManager:
    ...
    def __init__(self, 
                 plugins_dir: Union[str, Path] = None,
    ...
    async def discover_plugins(self, search_paths: List[Union[str, Path]] = None) -> ToolResponse:
        """
        ...
        try:
            ...
            return ToolResponse(
                success=False,
                data={},
                error=f"Plugin discovery failed: {str(e)}"
            )
    ...
    async def install_plugin(self, 
                           ...
                           verify_signature: bool = True) -> ToolResponse:
        """
    ...
    async def load_plugin(self, plugin_id: str, config: Dict[str, Any] = None) -> ToolResponse:
        """
        ...
        try:
            ...
            return ToolResponse(
                success=False,
                data={},
                error=f"Plugin loading failed: {str(e)}"
            )
    ...
    async def _load_plugin_module(self, plugin_info: PluginInfo) -> ToolResponse:
        ...
        try:
            ...
            # Find module file
            module_path = plugin_info.module_path / f"{module_name.replace('.', '/')}.py"
            ...
            spec = importlib.util.spec_from_file_location(
                f"plugin_{plugin_info.plugin_id}_{module_name}",
                module_path
            )
                ...
                if manifest.plugin_type == PluginType.AGENT:
                    if not issubclass(entry_obj, BaseAgent):
                        return ToolResponse(
                            success=False,
                            data={},
                            error=f"Agent plugin must inherit from BaseAgent"
                        )
                    ...
                    if isinstance(entry_obj, type):
                        plugin_info.instance = entry_obj(plugin_info.plugin_id, plugin_info.config)
                    else:
                        plugin_info.instance = entry_obj
        ...
        try:
            with self._plugin_lock:
                ...
                if plugin_id in self._reverse_deps:
                    dependent_plugins = [
                        pid for pid in self._reverse_deps[plugin_id]
                        if self._plugins[pid].state in [PluginState.LOADED, PluginState.ACTIVE]
                    ]
            ...
            return ToolResponse(
                success=False,
                data={},
                error=f"Plugin unloading failed: {str(e)}"
            )
    ...
    async def activate_plugin(self, plugin_id: str) -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse(
                success=False,
                data={},
                error=f"Plugin activation failed: {str(e)}"
            )
    ...
    async def get_plugin_info(self, plugin_id: str) -> ToolResponse:
        ...
        info_dict['instance'] = str(type(plugin_info.instance)) if plugin_info.instance else None
    ...
    async def get_manager_stats(self) -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse(
                success=True,
                data=stats,
                metadata={"operation": "get_manager_stats"}
            )
