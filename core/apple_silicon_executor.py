...
from .background_executor import (
    BackgroundAgentExecutor, ResourceLimits, BackgroundAgentInfo, 
    BackgroundStatus, EventBus, EventBusMessage
)
...
class AppleSiliconBackgroundExecutor(BackgroundAgentExecutor):
    ...
    def __init__(self, max_parallel_agents: int = None, config: AppleSiliconConfig = None):
    ...
    def _calculate_thread_efficiency(self) -> float:
        ...
        active_threads = len([a for a in self.active_agents.values() if a.execution_mode == "thread"])
        active_processes = len([a for a in self.active_agents.values() if a.execution_mode == "process"])
    ...
    async def optimize_for_workload(self, workload_type: str):
        ...
        if workload_type == "citation_processing":
            ...
            if self.max_parallel_agents < 25:
                self.max_parallel_agents = min(25, self._detect_apple_silicon_optimal_limit())
