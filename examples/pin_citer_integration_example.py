...
class PinCiterWorkflowOrchestrator_BEFORE:
    ...
    async def process_document(self, document: Document) -> List[Citation]:
        ...
        print(
            f"✅ BEFORE: Completed in ~7 seconds with {len(validated_citations)} citations"
        )
...
class PinCiterWorkflowOrchestrator_AFTER(BundleEnabledAgent):
    ...
    async def process_document(self, document: Document) -> List[Citation]:
        ...
        citation_extraction_task = (
            await self.background_executor.spawn_background_agent(
        ...
        print(
            f"✅ Launched 3 background agents: {doc_analysis_task}, {citation_extraction_task}, {zotero_search_task}"
        )
        ...
        print(
            f"✅ AFTER: Completed with concurrent processing + {len(validated_citations)} citations"
        )
...
async def main():
    ...
    print(
        f"BEFORE: {len(before_citations)} citations in {before_time:.1f}s (sequential)"
    )
    print(
        f"AFTER:  {len(after_citations)} citations in {after_time:.1f}s (concurrent + context preservation)"
    )
