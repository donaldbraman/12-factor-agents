...
class OptimalLimitTester:
    ...
    def analyze_system_health(self, snapshot: SystemSnapshot) -> Dict[str, Any]:
        ...
        if snapshot.memory_available_gb < 1.0:
            health_score -= 25
            warnings.append(f"Low available memory: {snapshot.memory_available_gb:.2f}GB")
        ...
        return {
            ...
            "status": "healthy" if health_score > 70 else "stressed" if health_score > 40 else "critical",
            "warnings": warnings
        }
    
    async def test_agent_limit(self, max_agents: int, test_duration: int = 30) -> TestResult:
        ...
        try:
            ...
            for i in range(max_agents):
                try:
                    ...
                    if spawn_time > 5.0:  # If spawning takes >5 seconds, system may be stressed
                        logger.warning(f"Slow spawn time: {spawn_time:.2f}s for agent {i}")
                ...
                if i % 5 == 0:  # Check every 5 agents
                    ...
                    if health["status"] == "critical":
                        logger.error(f"System critical during spawn {i}: {health['warnings']}")
            ...
            if not system_unresponsive and successful_spawns > 0:
                logger.info(f"Monitoring system with {successful_spawns} active agents...")
                ...
                while time.time() - monitor_start < test_duration:
                    ...
                    if response_time > 1.0:  # If basic operations take >1s, system is struggling
                        logger.warning(f"System unresponsive: basic operation took {response_time:.3f}s")
                        system_unresponsive = True
                        break
                        
                    if health["status"] == "critical":
                        logger.error(f"System critical during execution: {health['warnings']}")
                        system_unresponsive = True
                        break
        ...
        recommendation = self._generate_recommendation(
            max_agents, successful_spawns, failed_spawns, peak_cpu, peak_memory, 
            system_unresponsive, avg_response_time
        )
    ...
    def _generate_recommendation(self, max_agents: int, successful: int, failed: int,
                               peak_cpu: float, peak_memory: float, unresponsive: bool,
                               avg_response: float) -> str:
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        print(f"Baseline System Health: {baseline_health['status']} ({baseline_health['health_score']:.1f}/100)")
        ...
        for limit in test_limits:
            try:
                ...
                print(f"  Successful spawns: {result.successful_spawns}/{result.max_agents}")
                ...
                if result.system_became_unresponsive or result.failed_spawns > result.successful_spawns:
    ...
    def _generate_final_recommendations(self) -> Dict[str, Any]:
        ...
        # Find optimal ranges
        optimal_results = [r for r in self.test_results if "OPTIMAL" in r.recommendation]
        good_results = [r for r in self.test_results if "GOOD" in r.recommendation or "OPTIMAL" in r.recommendation]
        safe_results = [r for r in self.test_results if not r.system_became_unresponsive and r.failed_spawns == 0]
        ...
        if safe_results:
            ...
            recommendations["conservative_reasoning"] = f"Safe limit with no failures or system stress"
        else:
            recommendations["conservative_limit"] = 5
            recommendations["conservative_reasoning"] = "Very conservative due to system constraints"
        ...
        if good_results:
            ...
            recommendations["optimal_reasoning"] = f"Best performance without major resource stress"
        else:
            recommendations["optimal_limit"] = recommendations["conservative_limit"]
            recommendations["optimal_reasoning"] = "Same as conservative due to system constraints"
            
        # Maximum recommendation (absolute limit before system becomes unresponsive)
        working_results = [r for r in self.test_results if not r.system_became_unresponsive]
        if working_results:
            max_working = max(r.max_agents for r in working_results)
            recommendations["maximum_limit"] = max_working
            recommendations["maximum_reasoning"] = f"Highest tested limit before system stress"
        else:
            recommendations["maximum_limit"] = recommendations["optimal_limit"]
            recommendations["maximum_reasoning"] = "Same as optimal due to early system stress"
