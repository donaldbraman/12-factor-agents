...
from core.context_optimizer import ContextOptimizer, ContextOptimizedAgent
...
class ContextOptimizationBenchmark:
    ...
    def create_cite_assist_workload(self) -> Dict[str, Any]:
        ...
        return {
            "legal_research": {
                "case_law": [f"Case {i}: " + "Legal precedent analysis " * 10 
                            for i in range(100)],
                "statutes": [f"Statute {i}: " + "Legislative text " * 8 
                           for i in range(50)],
                "regulations": [f"Reg {i}: " + "Regulatory framework " * 6 
                              for i in range(30)]
            },
            "citation_scoring": {
            ...
            "zotero_integration": {
                "library_items": [f"Item_{i}: Research paper abstract" for i in range(200)],
                "collections": [f"Collection_{i}: Category description" for i in range(20)],
    ...
    def create_pin_citer_workload(self) -> Dict[str, Any]:
        ...
        return {
            "pdf_metadata": {
                ...
                "extracted_text": ["Page text content with citations " * 50 
                                 for _ in range(100)]
            },
            "processing_pipeline": {
                "stages": ["download", "extract", "chunk", "embed", "index", "validate"],
            ...
            "citation_extraction": {
                ...
                "references": [f"Reference_{i}: Full bibliographic entry" for i in range(150)],
                "footnotes": [f"Footnote_{i}: Explanatory note" for i in range(100)]
            },
            "conversation_history": [f"Processing status update {i}" for i in range(200)],
    ...
    def create_complex_workload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conversation_history": [
                f"Discussion {i}: Technical design conversation"
                for i in range(300)
            ],
    ...
    def run_benchmark(self, workload_name: str, context: Dict[str, Any], 
                     task: str, max_tokens: int = 8000) -> BenchmarkResult:
    ...
    def print_result(self, result: BenchmarkResult):
        ...
        if 40 <= result.reduction_percentage <= 60:
            ...
            print(f"  ⚠️  Outside target range (40-60%): {result.reduction_percentage:.1f}%")
    ...
    def print_summary(self):
        ...
        print(f"Target Achievement (40-60% reduction):")
        ...
        if avg_reduction >= 40 and avg_reduction <= 60:
            print("✅ OVERALL TARGET MET: Average reduction of {:.1f}% is within 40-60% target range".format(avg_reduction))
        else:
            print("⚠️  Average reduction of {:.1f}% is outside 40-60% target range".format(avg_reduction))
        
        # Performance analysis
        print()
        print("Performance Analysis:")
        print(f"  Tokens saved:  {sum(r.original_tokens - r.reduced_tokens for r in self.results):,}")
        print(f"  Throughput:    {sum(r.original_tokens for r in self.results) / sum(times):.0f} tokens/second")
        ...
        print(f"Delegation triggered: {delegations}/{total} workloads ({(delegations/total)*100:.0f}%)")
