...
def analyze_system():
    ...
    conservative_limit = calculate_conservative_limit(cpu_count_physical, memory_gb, system_type)
    ...
    print(f"🎯 AGENT LIMIT RECOMMENDATIONS")
    print(f"   📊 CONSERVATIVE: {conservative_limit} agents")
    print(f"      └─ Safe for production, low resource usage")
    print(f"   🚀 OPTIMAL: {optimal_limit} agents") 
    print(f"      └─ Best performance without system stress")
    print(f"   ⚠️  MAXIMUM: {maximum_limit} agents")
    print(f"      └─ Absolute limit before potential throttling")
    print()
    ...
    print(f"   Memory Utilization: {(parallel_memory_usage_gb / memory_gb * 100):.1f}% of total RAM")
    print()
    
    # Warnings and recommendations
    warnings = generate_warnings(current_cpu, current_memory, memory_available_gb, optimal_limit, system_type)
    ...
    print(f"   • High-Performance Bursts: Up to {maximum_limit} agents (monitor closely)")
    ...
    print(f"   ```python")
    print(f"   self.background_executor = BackgroundAgentExecutor(")
...
def generate_warnings(current_cpu, current_memory, available_memory_gb, optimal_limit, system_type):
    """Generate system warnings"""
    warnings = []
    
    if current_cpu > 80:
        warnings.append(f"High current CPU usage ({current_cpu:.1f}%) - consider fewer agents")
        
    if current_memory > 85:
        warnings.append(f"High current memory usage ({current_memory:.1f}%) - reduce agent count")
        
    if available_memory_gb < 2:
        warnings.append(f"Low available memory ({available_memory_gb:.1f}GB) - use conservative limits")
        
    if system_type == "laptop" and optimal_limit > 20:
        warnings.append("Laptop systems may throttle with high agent counts - monitor temperature")
