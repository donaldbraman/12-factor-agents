...
from core.context_bundles import BundleEnabledAgent, ActionType
...
class TestAgentWithWork(BundleEnabledAgent):
    ...
    async def _execute_task_impl(self, task: str) -> ToolResponse:
        ...
        return ToolResponse(
            success=True, data={"task": task, "phases_completed": len(phases)}
        )
...
class HandoffPerformanceBenchmark:
    ...
    async def benchmark_handoff_scenario(
        self, scenario_name: str, source_agent: BundleEnabledAgent
        ...
        handoff_session = await source_agent.create_handoff(
    ...
    def detect_context_loss(
        self, source: BundleEnabledAgent, target: BundleEnabledAgent
    ) -> bool:
    ...
    def print_result(self, result: HandoffBenchmarkResult):
        ...
        print(
            f"    Bundle size:     {result.bundle_size_mb:.2f} MB ({result.bundle_size_bytes:,} bytes)"
        )
        ...
        if result.total_recovery_time <= 3.0:
            print(
                f"    ✅ TARGET MET: Recovery time {result.total_recovery_time:.3f}s ≤ 3.0s"
            )
        else:
            print(
                f"    ⚠️  TARGET MISSED: Recovery time {result.total_recovery_time:.3f}s > 3.0s"
            )

        if not result.context_loss:
            print("    ✅ ZERO CONTEXT LOSS verified")
        else:
            print("    ⚠️  CONTEXT LOSS detected")
    ...
    async def test_concurrent_handoffs(self):
        ...
        for i, agent in enumerate(agents):
            agent.bundle_manager.update_state(
                {
                    "agent_id": i,
                    "work_data": [f"item_{j}" for j in range(20)],
                    "progress": i * 0.2,
                }
            )
        ...
        print("    ✅ All concurrent handoffs successful")
        print()
    ...
    async def test_handoff_chain_performance(self):
        ...
        for i in range(len(agents) - 1):
            ...
            # Create next handoff
            current_session = await source.create_handoff(
                f"chain_agent_{i+1}", {"chain_step": i}
            )
        ...
        print("    ✅ Handoff chain completed successfully")
        print()
    ...
    def print_summary(self):
        ...
        print(
            f"  Zero context loss: {context_target_met}/{len(self.results)} scenarios"
        )
        print(
            f"  Success rate: {(recovery_target_met * context_target_met) / len(self.results)**2 * 100:.0f}%"
        )
        print()
        ...
        if avg_recovery <= 3.0 and context_losses == 0:
            ...
            print("   • Zero context loss in all scenarios")
        ...
        print(
            f"  Bundle size efficiency: {avg_size/total_actions*1000:.3f} MB per 1K actions"
        )
