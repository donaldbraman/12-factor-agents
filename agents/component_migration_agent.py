...
class FileMigrationTool(Tool):
    ...
    def __init__(self):
        super().__init__(
            name="migrate_file", description="Copy or move a file to new location"
        )

    def execute(
        self, source: str, destination: str, operation: str = "copy"
    ) -> ToolResponse:
        """Migrate a file"""
        try:
            src = Path(source)
...
class ComponentMigrationAgent(BaseAgent):
    ...
    def execute_task(self, task: str) -> ToolResponse:
        ...
        for source, destination in migrations:
            ...
            if dst_path.exists():
                results.append(
                    {"file": destination, "status": "already_exists", "skipped": True}
                )
                continue

            # Try to migrate
            result = migrate_tool.execute(
                source=source, destination=str(dst_path), operation="copy"
            )

            if result.success:
                results.append(
                    {"file": destination, "status": "migrated", "from": source}
                )
            else:
                results.append(
                    {"file": destination, "status": "failed", "error": result.error}
                )
        ...
        if final_verify.success:
            ...
            return ToolResponse(
                ...
                data={"migrations": results, "verification": final_verify.data},
            )
