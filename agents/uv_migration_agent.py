...
class FileUpdaterTool(Tool):
    ...
    def __init__(self):
        super().__init__(
            name="update_file", description="Update a file with uv-specific changes"
        )

    def execute(
        self, file_path: str, replacements: List[Tuple[str, str]]
    ) -> ToolResponse:
        ...
        try:
            ...
            for old_pattern, new_pattern in replacements:
                if old_pattern in content:
                    ...
                    changes_made.append(
                        f"Replaced '{old_pattern[:30]}...' with '{new_pattern[:30]}...'"
                    )
            ...
            if content != original_content:
                ...
                return ToolResponse(
                    success=True,
                    data={"file": str(path), "changes": changes_made, "modified": True},
                )
            else:
                return ToolResponse(
                    success=True,
...
class ShebangUpdaterTool(Tool):
    ...
    def execute(self, directory: str, pattern: str = "*.py") -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse(
                success=True,
                data={"updated_files": updated_files, "count": len(updated_files)},
            )
...
class DocumentationUpdaterTool(Tool):
    ...
    def execute(self, doc_path: str) -> ToolResponse:
        ...
        try:
            ...
            if not path.exists():
                return ToolResponse(
                    success=False, error=f"Documentation file not found: {doc_path}"
                )
            ...
            replacements = [
                ...
                # Specific patterns
                (
                    'Path.home() / "Documents" / "GitHub"/12-factor-agents/bin/agent',
                    'uv run Path.home() / "Documents" / "GitHub"/12-factor-agents/bin/agent',
                ),
            ...
            if content != original:
                ...
                return ToolResponse(
                    success=True,
                    data={"file": str(path), "changes": changes, "modified": True},
                )
            else:
                return ToolResponse(
                    success=True,
...
class UvMigrationAgent(BaseAgent):
    ...
    def execute_task(
        ...
        shebang_result = shebang_tool.execute(directory=str(base_path), pattern="*.py")
...
"""
                ...
                (
                    ...
                    ToolResponse(
                        ...
                        data={
                            ...
                            "message": "Created pyproject.toml for uv",
                        },
                    ),
                )
            )
        ...
        modified_files = sum(
            1 for _, r in results if r.success and r.data.get("modified", False)
        )
    ...
    def _apply_action(self, action: Dict[str, Any]) -> ToolResponse:
        ...
        if action_type == "migrate":
            ...
            return ToolResponse(
                success=all(check[1] for check in checks), data={"checks": dict(checks)}
            )
...
if __name__ == "__main__":
    ...
    if result.success:
        ...
        print(
            f"Files modified: {result.data['files_modified']}/{result.data['total_files_checked']}"
        )
        print("\nModified files:")
