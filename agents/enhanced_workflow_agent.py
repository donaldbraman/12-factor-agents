...
class EnhancedWorkflowAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = None):
        ...
        self.orchestrator = ProgressAwareOrchestrator(f"enhanced_workflow_{self.agent_id}")
        ...
        logger.info(f"EnhancedWorkflowAgent initialized with {len(self.pipeline.stages)} pipeline stages")
    ...
    async def _processing_phase(self, workflow_data: Dict):
        ...
        logger.info(f"Processing completed. {len(successful)}/{len(results)} successful")
    ...
    def execute_task(self, task: str) -> ToolResponse:
        ...
        try:
            ...
            result = asyncio.run(
                self.orchestrator.start_workflow_async(task_params)
            )
