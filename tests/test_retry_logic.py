...
class TestRetryLogicValue:
    ...
    def test_file_lock_contention_with_retry(self):
        ...
        class FileToolWithRetry:
            ...
            def write(self, path: str, content: str, max_retries: int = 3) -> ToolResponse:
                for attempt in range(max_retries):
                    try:
                        self.attempts += 1
                        if self.attempts < 3:
...
class TestRetryConfiguration:
    ...
    def test_configurable_retry_strategies(self):
        ...
        file_delays = [calculate_delay(strategies["file_operation"], i) for i in range(4)]
        assert file_delays == [0, 0.1, 0.2, 0.4]
        
        # Test network operation delays (slower, longer backoff)
        net_delays = [calculate_delay(strategies["network_operation"], i) for i in range(4)]
        ...
        crit_delays = [calculate_delay(strategies["critical_operation"], i) for i in range(2)]
