...
class TestAgentRegistry:
    ...
    async def test_register_agent_success(
        self, registry, sample_agent_metadata, temp_dir, sample_agent_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_invalid_version(
        self, registry, sample_agent_metadata, temp_dir, sample_agent_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_nonexistent_file(
        self, registry, sample_agent_metadata
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_invalid_class(
        self, registry, sample_agent_metadata, temp_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_discover_agents_by_name(
        self, registry, sample_agent_metadata, temp_dir, sample_agent_code
    ):
        ...
        await registry.register_agent(
            sample_agent_metadata, str(agent_file), "TestAgent"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_agent_rating(
        self, registry, sample_agent_metadata, temp_dir, sample_agent_code
    ):
        ...
        # Update rating
        rating_result = await registry.update_agent_rating(
            agent_id, 4.5, "Great agent!"
        )
...
class TestPluginManager:
    ...
    async def test_discover_plugins(
        self, plugin_manager, temp_dir, sample_plugin_manifest
    ):
    ...
    @pytest.mark.asyncio
    async def test_install_plugin_from_directory(
        self, plugin_manager, temp_dir, sample_plugin_manifest, sample_agent_code
    ):
        ...
        (source_dir / "test_plugin.py").write_text(
            sample_agent_code.replace("TestAgent", "TestPlugin")
        )
    ...
    async def test_load_plugin_success(
        ...
        await self._install_test_plugin(
            plugin_manager, temp_dir, sample_plugin_manifest, sample_agent_code
        )
    ...
    @pytest.mark.asyncio
    async def test_activate_plugin(
        self, plugin_manager, temp_dir, sample_plugin_manifest, sample_agent_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_plugin_info(
        self, plugin_manager, temp_dir, sample_plugin_manifest, sample_agent_code
    ):
    ...
    async def _install_test_plugin(self, plugin_manager, temp_dir, manifest, code):
        ...
        (source_dir / "plugin.json").write_text(json.dumps(manifest_dict))
        (source_dir / "test_plugin.py").write_text(
            code.replace("TestAgent", "TestPlugin")
        )
...
"""
        ...
        assert any(
            "banned_import" in vt or "dangerous_import" in vt for vt in violation_types
        )
        assert any(
            "eval_exec" in vt or "code_injection" in vt for vt in violation_types
        )
...
"""
        ...
        # Audit agent
        result = await security_manager.audit_agent(
            agent_file, security_level=SecurityLevel.STANDARD
        )
...
"""
    ...
    def test_verify_invalid_signature(self, security_manager, temp_dir):
        ...
        verify_result = security_manager.verify_signature(
            test_file, "invalid_signature"
        )
        assert verify_result.success
...
class TestIntegration:
    ...
    async def test_full_agent_lifecycle(
        self, temp_dir, sample_agent_metadata, sample_agent_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_plugin_with_security_validation(
        self, temp_dir, sample_plugin_manifest, sample_agent_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_marketplace_stats_and_reporting(
        self, temp_dir, sample_agent_metadata, sample_agent_code
    ):
        ...
        for i in range(3):
            ...
            agent_file.write_text(
                sample_agent_code.replace("TestAgent", f"TestAgent{i}")
            )
...
"""
            ...
            tasks.append(
                registry.register_agent(metadata, str(agent_file), f"PerfAgent{i}")
            )
