...
class TestEventBus:
    ...
    async def test_publish_events(self, event_bus):
        ...
        message = EventBusMessage(
            agent_id="test_agent", event_type="agent_spawned", data={"test": "data"}
        )
    ...
    async def test_async_event_callbacks(self, event_bus):
        ...
        message = EventBusMessage(
            agent_id="async_agent", event_type="async_event", data={}
        )
    ...
    def test_get_latest_events(self, event_bus):
        ...
        for i in range(5):
            message = EventBusMessage(
                agent_id="test_agent", event_type="progress_update", data={"step": i}
            )
        ...
        other_message = EventBusMessage(
            agent_id="other_agent", event_type="progress_update", data={"step": 0}
        )
...
"""
        ...
        result = BackgroundAgentExecutor._execute_agent_script(
            str(test_script), ResourceLimits(max_execution_time_minutes=1)
        )
...
class TestBackgroundEnabledAgent:
    ...
    async def test_spawn_background_task(self, agent):
        ...
        with patch.object(
            agent.background_executor, "spawn_background_agent"
        ) as mock_spawn:
            mock_spawn.return_value = "bg_agent_123"

            # Spawn background task
            bg_agent_id = await agent.spawn_background_task(
                task="background test task", workflow_data={"bg": True}
            )
    ...
    async def test_wait_for_completion_timeout(self, agent):
        ...
        with patch.object(agent, "check_background_status") as mock_status:
            ...
            result = await agent.wait_for_background_completion(
                "bg_agent_123", timeout=0.1
            )
    ...
    async def test_terminate_background_agent(self, agent):
        ...
        # Mock termination
        with patch.object(
            agent.background_executor, "terminate_agent"
        ) as mock_terminate:
