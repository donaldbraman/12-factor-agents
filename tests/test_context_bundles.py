...
class TestContextBundleManager:
    ...
    def test_append_only_logging(self, manager):
        ...
        manager.append_action(
            ActionType.TASK_EXECUTION, {"task": "start implementation"}, "agent_1"
        )
        ...
        manager.append_action(ActionType.STATE_CHANGE, {"progress": 0.2}, "agent_1")
    ...
    def test_state_updates(self, manager):
        ...
        state_changes = [
            a for a in manager.append_log if a.action_type == ActionType.STATE_CHANGE
        ]
    ...
    def test_checkpoint_creation(self, manager):
        ...
        checkpoint_actions = [
            a for a in manager.append_log if a.action_type == ActionType.CHECKPOINT
        ]
    ...
    def test_context_remounting(self, manager):
        ...
        manager.append_action(
            ActionType.TASK_EXECUTION, {"task": "original"}, "agent_1"
        )
        ...
        assert (
            len(
                [
                    a
                    for a in manager.append_log
                    if a.action_data.get("task") == "original"
                ]
            )
            >= 1
        )
    ...
    def test_action_history_filtering(self, manager):
        ...
        limited_task_actions = manager.get_action_history(
            ActionType.TASK_EXECUTION, limit=1
        )
...
class TestBundleEnabledAgent:
    ...
    class TestAgent(BundleEnabledAgent):
        ...
        async def _execute_task_impl(self, task: str) -> ToolResponse:
            ...
            return ToolResponse(
                success=True, data={"result": f"Completed task: {task}"}
            )
    ...
    async def test_handoff_creation(self, agent):
        ...
        agent.bundle_manager.update_state(
            {"current_work": "implementation", "progress": 0.6}
        )
    ...
    async def test_handoff_reception(self, agent):
        ...
        source_agent = self.TestAgent(
            agent_id="source_agent", session_id="source_session"
        )
        ...
        handoff_session_id = await source_agent.create_handoff(
            "test_agent", handoff_data
        )
        ...
        receipt_actions = [
            a
            for a in handoff_actions
            if a.action_data.get("event") == "handoff_received"
        ]
