...
class TestDynamicPrimerIntegration:
    ...
    def test_end_to_end_feature_development_workflow(self):
        ...
        assert all(result.generation_time < 1.0 for result in [feature_result, testing_result, doc_result])
    ...
    def test_cli_create_command(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            test_args = [
                ...
                'create',
                'feature_development',
                '-v', 'task_description=Test CLI task',
                '-v', 'priority=High',
                '-o', str(output_file)
            ]
    ...
    def test_multi_primer_workflow_chain(self):
        ...
        all_results = [research_result, feature_result, testing_result, optimization_result]
        assert all(r.success for r in all_results)
        
        # Content should be related across primers
        cache_mentions = sum(
            1 for result in all_results 
            if 'cach' in result.content.lower()
        )
        ...
        def run_cli_create(primer_type, task_id):
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                test_args = [
                    ...
                    'create',
                    primer_type,
                    '-v', f'task_description=Concurrent task {task_id}',
                    '-o', str(output_file)
                ]
    ...
    def test_primer_system_with_real_12_factor_workflow(self):
        ...
        assert all(r.generation_time < 0.5 for r in [research_result, feature_result, testing_result])
