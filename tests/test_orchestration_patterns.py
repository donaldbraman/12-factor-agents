...
class TestPipelinePattern:
    ...
    async def test_pipeline_stage_processing(self):
        ...
        string_result = await pattern._execute_pipeline_stage("agent", "input", "stage_1")
        assert "stage_1" in string_result
        
        # Test dict processing
        dict_input = {"key": "value"}
        dict_result = await pattern._execute_pipeline_stage("agent", dict_input, "stage_2")
        assert "stage_2" in dict_result
        assert dict_result["key"] == "value"
        
        # Test list processing
        list_input = ["item1", "item2"]
        list_result = await pattern._execute_pipeline_stage("agent", list_input, "stage_3")
...
class TestScatterGatherPattern:
    ...
    async def test_gather_responses(self):
        ...
        responses = [
            ...
            {"agent_id": 2, "response": "Response A", "confidence": 0.7}  # Same as first
        ]
...
class TestPatternExecutor:
    ...
    def test_performance_metrics_retrieval(self):
        ...
        for pattern_name in ["mapreduce", "pipeline", "fork_join", "scatter_gather", "saga"]:
...
class TestBaseOrchestrationPattern:
    ...
    def test_performance_metrics(self):
        ...
        results = [
            PatternResult(OrchestrationPattern.FORK_JOIN, True, execution_time=1.0, agents_used=2),
            PatternResult(OrchestrationPattern.FORK_JOIN, True, execution_time=2.0, agents_used=3),
            PatternResult(OrchestrationPattern.FORK_JOIN, False, execution_time=0.5, agents_used=1)
        ]
